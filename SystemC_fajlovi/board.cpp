...
//int attack_count=0;//dejan
BOOL attack(int sq, int s)      //Dejan:It question if it is Chess
{
	int i, j, n;
    //printf("Attack %d \n\n", attack_count);
    //attack_count++;
...
for_i:  if(i<64)
        {
            if (color[i] == s) {
                ...
                {
                    j = 0;
                    for_j:  if (j < offsets[piece[i]])
                            {
                                n = i;
                                for_n:  n = mailbox[mailbox64[n] + offset[piece[i]][j]];
                                        if (n == sq)
                                            return TRUE;
                                        else if (n == -1 || color[n] != EMPTY || slide[piece[i]] == FALSE)
                                            goto break_n;     //Break from for_n
                                        else
                                            goto for_n;     //Continue for_n
...
BOOL makemove(move_bytes m)
{
	...
	if (m.bits & 2) {
		...
		if (in_check(side))
			return FALSE;
		switch (m.to) {
			case 62:    //Place to move King to castle G1
				...
				to = F1;
				break;
			case 58:    //Place to move King to castle C1
				...
				to = D1;
				break;
			case 6:    //Place to move King to castle G8
				...
				to = F8;
				break;
			case 2:    //Place to move King to castle C8
	...
	if (m.bits & 8) {
		if (side == LIGHT)
			ep = m.to + 8;  //ep- en passant
		else
			ep = m.to - 8;
	}
	else
		ep = -1;
	if (m.bits & 17)    //pawn move + capture
		fifty = 0;
	else
		++fifty;
